...
- REMOVED. This view used to call
  megamind.services.connected_service_sync.sync_product_connected_service()
  after every successful save, to keep a mirrored ConnectedService row
  in sync with product edits. That call has been intentionally removed
  — editing a product no longer creates or updates a ConnectedService
  row as a side effect. Product and ConnectedService are independent
  here, matching product_upload.py's create path.
  If a product ever needs a mirrored ConnectedService row again, call
  sync_product_connected_service() explicitly wherever that's actually
  needed — it's unchanged and still available, this view just no
  longer calls it automatically on edit.
