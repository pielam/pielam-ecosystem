...
def SubCategoryEditView(request, pk):
    ...
    Fetches the existing SubCategory by pk, pre-populates the form,
    ...
    - On success redirects to ponno:home (update to your list view when ready).
    """
    user = request.user
    ...
    # Fetch the existing subcategory — reject soft-deleted ones
    ...
    if request.method == 'POST':
        try:
            with transaction.atomic():
                ...
                # Commission rate
                commission_rate = sub_category.commission_rate
                ...
                # ===== 4. APPLY CHANGES TO EXISTING INSTANCE =====
                ...
                if sub_category_icon_file: sub_category.sub_category_icon      = sub_category_icon_file
                if sub_category_thumbnail: sub_category.sub_category_thumbnail = sub_category_thumbnail

                # ===== 5. SAVE (triggers full_clean; slug is intentionally preserved) =====
                sub_category.save()
